...
export class S3Store extends DataStore {
    ...
    async create(req): Promise<File> {
        ...
        if (upload_length === undefined && upload_defer_length === undefined) {
            throw Promise.reject(ERRORS.INVALID_LENGTH);
        }
        ...
        try {
            ...
            throw Promise.reject(ERRORS.FILE_WRITE_ERROR);
        }
    ...
    async getOffset(file_id, with_parts: boolean = false): Promise<{ [key: string]: number }> {
        return new Promise((resolve, reject) => {
            this._getMetadata(file_id)
                .then((metadata) => {
                    return this._retrieveParts(file_id)
